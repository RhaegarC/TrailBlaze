...
| 10 | [figma-integration](10-figma-integration.md) | 05, 07, 08, 09 | The Figma-exported app wired to the API: MSAL login, API calls, role gating, **visibility badges**, **media grouped by uploader**, and the **profile screen**. **Non-TDD** — verified manually | in progress — branch `feature/10-figma-integration`, under review in PR #31. The export's screens are untouched: the wiring is `src/api/`, `src/auth/`, `src/data/`, `src/config.ts` and marked seams in `App.tsx`, and `scripts/web-seam.py` now fails when a re-export clobbers one of those files without saying so. **Three criteria are not met and are not claimed** — the export renders no `<video>` element, no per-media delete control, and no avatar an anonymous visitor can see, so all three are recorded as export gaps rather than hand-built, per the no-hand-authored-screens non-goal. **The browser walkthrough has not been run**, so no criterion is claimed on the strength of having seen it work; that pass is [11](11-e2e-verification.md)'s |
