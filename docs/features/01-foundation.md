...
Status: **In progress** — scaffold present, test harness outstanding · [00-mission-1-sprint.md](00-mission-1-sprint.md)
...
sibling xUnit project per layer, EF Core against **Azure SQL Server** with migrations applied at
startup, a `docker-compose` that brings up the API and an Azure SQL Server container together, the
...
already exists and part does not. Do not rebuild the first column:

| Already present | Outstanding |
|---|---|
| The five layers under `src/api/`, flat, with `TrailBlaze.slnx` at that level | The three `*.Test` projects are **empty and reference no project under test** — `dotnet test` discovers zero tests |
| EF Core registered through `AddRepositoryPersistence` with the connection string read from configuration; migrations and a model snapshot exist | The provider is **PostgreSQL (`Npgsql`)**, not Azure SQL Server |
| `GET /health` and the OpenAPI document (development only) | No `docker-compose` file and no CI workflow |
| `EntityBase`, `AuditSaveChangesInterceptor` and the soft-delete query filter | No `IStorageService`, and no in-memory fake |
| Entra bearer validation and caller auto-provisioning — but that is feature 02's subject; see [02-entra-auth.md](02-entra-auth.md) | No Azure Blob configuration key |

Two items gate every other feature: **the test harness** and **the provider swap**. Until the
harness runs a real assertion, no later feature has a RED step to begin from; until the provider
is Azure SQL Server, every migration written here is shaped for the wrong database.
...
- [ ] Each `TrailBlaze.<Layer>.Test` project references the layer it exercises, and `dotnet test`
...
- [ ] The repository tier follows the no-database pattern of
...
- [ ] `TrailBlaze.Api` registers EF Core against **Azure SQL Server** through a `DbContext` whose
