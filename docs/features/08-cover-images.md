...
- **Hot spot (container routing — test-first)** — `TrailBlaze.Repository.Test` against a live account, **tier unsettled** (the wiring note at the end of this bullet is the decision that settles it): the destination container as a table over `Type` — `covers` for `Public`, `media` for `Shared` and `Private` — asserted by outcome, per type rather than once, because a single happy-path check passes even when the routing is inverted for the other two. **Changed 2026-09-18: this is no longer a `TrailBlaze.Service.Test` unit test**, because the observation it needs no longer exists there — the fake that reported which container was asked for is deleted, and a real backend reports nothing about the call. The outcome assertion is strictly stronger than the argument comparison it replaces (it says where the bytes are, not where the caller meant them to go); the claim it gives up is the one the repository tier's own routing suite names — a wrong object moved to the right place. One wiring decision this creates, stated so the feature makes it deliberately: routing is decided in the service layer, and `TrailBlaze.Service.Test` does not reference the repository tier, so hosting the table against a real account means pointing the service tier at one — the same unsettled wiring as feature 06's count cap, and a gap if it is left.
- **Hot spot (the move — test-first)** — `TrailBlaze.Repository.Test` against a live account, tier unsettled as above: `Public` → `Private` copies into the private container **and deletes the public blob**; the reverse direction likewise. Also that `Shared` ↔ `Private` moves nothing, and that a `Type` change with no cover does no storage work. **Changed 2026-09-18:** "asserted as two calls on the fake, with the delete asserted explicitly" is withdrawn with the fake. The move has a real end-to-end equivalent and it is the better one — copy-then-delete through the real `AzureBlobStorageRepository`, asserting the object **arrives** in the destination container and is **gone** from the source, which is the thing the two-call count was standing in for (and it carries the same service-tier wiring as the routing bullet above). The no-op branches are asserted on observable state rather than on call counts: the stored path is unchanged and no object appears in either container. Be plain about the cost, because the no-op half is where it lands: "no storage call was made" is a negative a real backend cannot report, so it is asserted as "nothing changed", which a caller that called storage and changed nothing back would also satisfy.
- `TrailBlaze.Service.Test` keeps the parts of this feature that are decided from values rather than from a store: the image allowlist accept/reject table including a video type, and the size boundary at exactly 10 MB and 10 MB + 1 byte. **Changed 2026-09-18:** "replacement deletes the previous cover blob and stores the new path" and "a rejected upload leaves the existing cover and its blob intact" are outcomes about a store and about the row that names it, so they move to the container tier and to the persistence tier respectively — the first as a real delete whose absence is observable by a credential-free GET against the old URL, the second as a read-back of `CoverImageBlobPath`.
- `TrailBlaze.Service.Test`: the URL shape per type — a plain public URL for `Public`, a SAS for `Shared`/`Private` — asserted on the value the service returns, which needs no store because the branch is decided from the activity's `Type` and the string either carries a signature or does not. **Changed 2026-09-18:** "asserted against the fake" is withdrawn with the fake, and what this check cannot show is the container tier's half: a well-formed SAS produced from the wrong key looks identical here and is only refused on fetch.
- Integration (`TrailBlaze.Repository.Test`) — **container-backed** (`Category=Container`): `CoverImageBlobPath` genuinely persists on the activity row — one database per test class, created with `Migrate()` and dropped on dispose, read back through a second context ([testing-and-tdd.md](../testing-and-tdd.md)) — and a cover upload leaves the activity's `media` rows unchanged, now as a real query rather than an inspection of generated SQL. **Withdrawn 2026-09-18: "with no database"**, which described the unreachable-connection harness: that could show EF had staged a value, not that the row held it. The model tier still inspects reads with `ToQueryString()` where the claim is about the statement.
...
- Storage integration (`TrailBlaze.Repository.Test`, tagged `Category=Container`; `dotnet test --filter "Category=Container"`): a real round-trip against the public container — upload a `Public` activity's cover through the real `AzureBlobStorageRepository`, then fetch the returned URL with a **credential-free** HTTP GET and receive 200 with the original bytes. That anonymous fetch is the proof the container is genuinely public-read, which no double can demonstrate. **2026-09-18:** the project and the deleted `Category=StorageIntegration` tag are corrected, and this proof is no longer conditional on credentials in CI — the tier runs against the Azurite emulator by default (the container's access level is set only when the endpoint is loopback, so the anonymous read is real), against a real account when `TRAILBLAZE_STORAGE_CONNECTION` names one, and it **skips, never fails**, when nothing answers.
- Storage integration (`TrailBlaze.Repository.Test`, tagged `Category=Container`) — **the move as a real round-trip, and the tier no double can replace:** upload a `Public` cover, change the activity to `Private`, then assert a credential-free GET against the **old** URL now fails while a fresh SAS succeeds. A recording double can prove the delete was *called*; only a real backend proves the bytes are actually unreachable, which is the property the whole rule exists to guarantee — and that gap is why this bullet is not negotiable. **2026-09-18:** project and trait corrected; the tier runs against the emulator by default and against a real Azure account only when `TRAILBLAZE_STORAGE_CONNECTION` names one, and it **skips, never fails**, when nothing answers.
